...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
    ...
    #pragma omp parallel
    {
        ...
        for(int i=current; i<ARRAY_SIZE; i+=block_size)
            z[i] = x[i] + y[i];
    }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
    ...
    #pragma omp parallel
    {
        int block_num = omp_get_num_threads();
        int block_size = ARRAY_SIZE/block_num;
        int current = omp_get_thread_num();
        int j = current;
        for (int i=current*block_size; i<(current+1)*block_size; i++) {
            z[i] = x[i] + y[i];
            
        }
        if (current==block_num-1) {
            for (int i=(current+1)*block_size; i < ARRAY_SIZE; i++) {
            z[i] = x[i] + y[i];
            }
        }    
    } 
